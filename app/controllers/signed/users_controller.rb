class Signed::UsersController < Signed::BaseController
  ...
  def update
   ...
   if @user.update_attributes(params[:user])
      @user.account_setting.update_attributes(:country_name => params[:country_name])
  ...
  def add_photos
    ...
    redirect_to :action => :manage, :form_type => "add_profile_photos"
  end
...
#  end
  ...
  def add_circle_photos
    ...
    if @circle and !params[:circle].blank?
      @circle.update_attributes(params[:circle])
      redirect_to :action => :manage, :form_type => "circle_info", :id => @circle.id, :sub_form => "add_photos"
    else
      redirect_to :action => :manage, :form_type => "circle_info", :id => @circle.id, :sub_form => "add_photos"
    end
  end
  ...
  def emotions_label
    @emotions_label = EmotionLabel.new(:emotion_name => params[:emotion_name], :emotion_order => params[:emotion_order], :user_id => current_user.id)
    @emotions_label.save
