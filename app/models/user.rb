class User
  ...
  #attributes for registration form
   ...
   #field :fb_profile_image, :type => String

  validates :birthday, :presence => {:message => "Site is restricted for you", :if => :should_be_filled_in?}
  ...
  before_create :set_domain #, :should_be_filled_in
  ...
  def should_be_filled_in?
    if Date.today.year - self['birthday(1i)'].to_i >= 12
       return false
    else
       return true
    end
    ...
    tire.search(page: params[:page], per_page: 10, load: true) do
      query do
        boolean do
          ...
          must { term :domain, params[:filter] } if params[:filter].present?
  ...
  def set_domain
    self.domain = self.login.email.split('@').last
  end
